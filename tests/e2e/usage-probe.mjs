// Usage/status-bar chips probe — the daemon started the way the
...
function startMock() {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/_server') {
      ...
      // exact "public actor" error shape seen without a session.
      const body = authed
    ...
    if (url.pathname === `/workspace/${WID}/go`) {
      ...
      // mirrors the provider's regexes, which require that exact shape.
      res.end(
