...
//   C9 status + context give another agent's live state AND full redacted chat.
...
`,
  ...
  // The kind fix: a worker SPAWNED as omp must report kind:omp on the roster, even though its
  // process tree only ever shows the codex engine omp boots. Read it two ways — the roster row and
  // the worker's own whoami — and confirm neither says codex.
  const rosterRow = (parseJson(cliSync(['roster', '--json'], tokenA, 30000).stdout)?.agents ?? []).find(
    (a) => a.id === worker || (typeof a.id === 'string' && worker.startsWith(a.id)),
  )
  // whoami of the WORKER ITSELF — derive its own token exactly as the daemon does, so this is the
  // omp worker reporting its own identity, not one of the seeded codex control terminals.
  const workerToken = createHmac('sha256', secret).update(worker).digest('hex')
  const who = parseJson(cliSync(['whoami', '--json'], workerToken, 30000).stdout) ?? {}
  const rosterKind = rosterRow?.kind ?? null
  const whoamiKind = who.kind ?? null
  log('kind', { rosterKind, whoamiKind })

  // Greet it, exactly as a peer would — the comms must still work byte-for-byte as before.
  ...
  const commsOk = gp.ok === true && (gp.channel === 'check' || gp.status === 'delivered')
  const kindOk = rosterKind === 'omp' && whoamiKind === 'omp'
  console.log('RESULT:', JSON.stringify({
    ok: commsOk && kindOk,
    commsOk,                         // comms unchanged by the label fix
    kindOk,                          // the bug: roster + whoami now say omp, not codex
