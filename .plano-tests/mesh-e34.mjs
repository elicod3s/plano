// Plan AGENT_MESH_INTERCONNECT E3/E4: two terminals, simulated harness verdicts, then:
//   E3 — A sends to B in mode 'type' → the text must appear in B's PTY (verified via the
//        daemon's replay buffer, since the renderer is WebGL and the DOM has no text)
//   E4 — B busy → queue mode holds the message in the mailbox; when B goes idle it is
//        delivered (appears in B's buffer)
...
/** TCP RPC to the daemon: hello handshake, then one method call, returns its result. */
...
async function main() {
  ...
  const UD = path.join(os.tmpdir(), `mesh-e34-${run}`)
  const PRJ = path.join(os.tmpdir(), `mesh-e34p-${run}`)
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      activeId: 's1',
      workspaces: [{ id: 's1', name: 'S', folderPath: PRJ, viewport: { x: 0, y: 0, zoom: 1 }, regions: [], panels: [mkTerm('t1', 60), mkTerm('t2', 520)] }],
    }),
  )
  ...
  const ptyA = sessions[0]?.ptyId
  const ptyB = sessions[1]?.ptyId
  ...
  const verdict = (ptyId, kind, phase) => rpc(host, 'reportVerdict', { ptyId, verdict: { active: true, kind, phase, displayName: kind } })
  console.log('STEP: verdicts')
  await verdict(ptyA, 'claude', 'idle')
  console.log('STEP: idle verdict')
  await verdict(ptyB, 'codex', 'idle')
  await sleep(800)

  // E3: A → B, mode type.
  const rosterBefore = await toolResult(webPort, tokenA, 'plano_roster', {})
  console.log('STEP: roster')
  // The FIRST mesh write in a workspace blocks on the one-click consent toast, so fire it
  // without awaiting, click Allow, then collect. Consent is remembered → later calls run free.
  const sendPending = post(webPort, tokenA, { jsonrpc: '2.0', id: 1, method: 'plano_send', params: { to: ptyB, text: 'hello from A', mode: 'type' } })
  ...
  const sendRaw = await sendPending
  console.log('STEP: consent granted=' + consentClicked)
  const sendResult = await toolResult(webPort, tokenA, 'plano_send', { to: ptyB, text: 'hello from A', mode: 'type' })
  console.log('STEP: buffer after type, sendResult=' + JSON.stringify(sendResult))
  await sleep(4500)
  const bufferAfterType = (await rpc(host, 'attach', { ptyId: ptyB }))?.result?.buffer ?? ''
  const xtermText = (await ev(`(() => { const ts=[...document.querySelectorAll('.xterm')]; return ts.map((t)=>(t.querySelector('.xterm-rows')?.textContent||'').slice(-80)) })()`)) || []
  const writeR = await rpc(host, 'write', { ptyId: ptyB, data: 'echo mesh-probe-test\r\n' })
  await sleep(1200)
  const bufferProbe = (await rpc(host, 'attach', { ptyId: ptyB }))?.result?.buffer ?? ''
  console.log('WRITE_RESULT:', JSON.stringify(writeR), '| ptyB:', ptyB, '| xterms:', JSON.stringify(xtermText))
  const typedSeen = bufferAfterType.includes('hello from A') && bufferAfterType.includes('from Claude')

  // E4: B busy → queue holds; B idle → delivered.
  await verdict(ptyB, 'codex', 'working')
  await sleep(600)
  const queueResult = await toolResult(webPort, tokenA, 'plano_send', { to: ptyB, text: 'queued hello', mode: 'queue' })
  console.log('STEP: queued, queueResult=' + JSON.stringify(queueResult))
  await sleep(600)
  const inboxA = await toolResult(webPort, tokenA, 'plano_inbox', {})
  await verdict(ptyB, 'codex', 'idle')
  await sleep(4500)
  const bufferAfterQueue = (await rpc(host, 'attach', { ptyId: ptyB }))?.result?.buffer ?? ''
  const queuedSeen = bufferAfterQueue.includes('queued hello')

  console.log(
    'RESULT:',
    JSON.stringify({
      roster: rosterBefore.agents,
      sendRaw: JSON.stringify(sendRaw).slice(0, 300),
      sendStatus: sendResult.status,
      sendError: sendResult.error,
      typedSeen,
      queueStatus: queueResult.status,
      queuedSeen,
      inboxACount: (inboxA.messages ?? []).length,
    }),
  )
