...
- **Installer is unreliable here:** the NSIS `Setup.exe` crashes on extraction (`0xC0000005`). Install by copying `release/win-unpacked` → `%LOCALAPPDATA%/Programs/PLANO` (`Copy-Item`/`cp`; note `robocopy` and `Copy-Item -Force` can be hook-blocked — use `cp -rf` via Bash). Faster too: `electron-builder --dir` + copy.
