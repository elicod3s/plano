...
## Environment gotchas (Linux / Fedora KDE — port-specific)

- **The repo is on an NTFS-fuse mount with `nosuid`.** Electron's `chrome-sandbox` needs the setuid bit; it cannot get it on `nosuid`, so `npm run dev` and packaged AppImages fail with "SUID sandbox helper binary is not configured correctly" when run from the NTFS path. Fix: work in a btrfs sandbox at `~/plano-build` — see `docs/engineering/LINUX_SETUP.md`. Never `--no-sandbox` a shipped build; it is dev-only.
- **`dist:linux` must chain `npm run build:web`.** Without it `web-dist/` is never rebuilt, so the `extraResources` copy of `web-dist → resources/web` ships stale or absent and PLANO Mobile is broken in the packaged app. (`dist:mac` is missing this too — pre-existing macOS issue, not ours to fix per RULE 0.)
- **node-pty rebuilds cleanly** with `gcc-c++` + `make` + `python3` + `@electron/rebuild` against Electron 33's ABI. `npm run rebuild` is the command. `npmRebuild: false` in the build config is correct — the local rebuild is what matters.
- **sherpa-onnx resolves its Linux binary** through the optional dependency `sherpa-onnx-linux-x64`. Verify it installed; the existing `asarUnpack: ["**/node_modules/sherpa-onnx-*/**"]` pattern covers packaging.
- **electron-builder `linux` block** targets AppImage only — the only Linux format `electron-updater` can auto-update. **The `rpm` target does NOT build on Fedora 44:** electron-builder 25 bundles fpm 1.9.3 (2017), and the spec it generates is rejected by `rpmbuild` 6.x (`Process failed: rpmbuild failed (exit code 1)`). The AppImage builds fine; only rpm dies, and it kills the whole run, so the target is off. Re-add it only with a working fpm (`USE_SYSTEM_FPM=1` plus a modern fpm, or build in a container with older rpm). The `desktop.StartupWMClass` is set to `plano` so KDE groups the window with its icon.
- **Line endings: git stores LF, the NTFS worktree is CRLF.** A Windows machine with `core.autocrlf=true` wrote the files; git's blobs stayed LF, so on Linux every file looked modified (~181 of them). Fixed with `git config core.autocrlf input` **in this repo only** — git strips CR when staging, never converts on checkout, and no file on disk was rewritten. `git status` dropped to only the real changes. Do not undo it, do not add a `.gitattributes`, do not run a normalization pass. Keep writing CRLF to match what is on disk. Note `daemon/agentHooks.ts` deliberately *emits* CRLF hook scripts at runtime (cmd.exe re-seeks a `.cmd` by byte offset; LF-only breaks it) — unrelated, never normalize that output.
